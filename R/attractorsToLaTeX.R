...
attractorsToLaTeX <- function (attractorInfo, grouping = list(), title = "", plotFixed = TRUE,
				onColor="[gray]{0.9}",offColor="[gray]{0.6}", file="attractors.tex")  
{
	...
	binMatrices <- lapply(attractorInfo$attractors,function(attractor)
					{
						...
						res <- matrix(apply(attractor$involvedStates,2,function(state)
							print(dec2bin(state,numGenes)[plotIndices])),nrow=length(plotIndices))
					})
	...
	# accumulate all attractors with equal length in one matrix and plot them
	{
		...
		# output table header
    		cat("\\begin{table}[ht]\n",
		       ...
		       paste(rep(paste(rep(">{\\centering\\arraybackslash}X", 
		            len), collapse = " "),cnt),collapse="|"), 
		    "}\\hline\n",
	     	    sep="")
		
		cat("\t&\t",paste(paste("\\multicolumn{",len,"}{c}{Attr. ",1:cnt,"}",
				  sep=""),collapse="\t&\t"),"\\\\\n")		
		...
		freq <- round(sapply(attractorInfo$attractors[which(attractorLengths == len)],
